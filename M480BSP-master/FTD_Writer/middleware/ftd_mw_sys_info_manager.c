...
{
    .writer_hw_ver  = "V00.02.00",
    .writer_sw_ver  = "V00.01.02",   // "V01.23.45" "01.23" is writer fw version, "45" is HMI fw version
...
{
    uint8_t b_need_save = 0;  // Flag to indicate if save is needed
    ...
    {
        char sw_temp_ver[WRITER_SW_VER_LEN];
        memcpy(sw_temp_ver, p_st_sys_info->st_writer_info.writer_sw_ver, sizeof(sw_temp_ver));
        endian_reverse_buff((uint8_t*)&sw_temp_ver, sizeof(sw_temp_ver));
        /* Check if sw_ver matches the default value, if not update it */
        if (memcmp(sw_temp_ver, const_writer_info_default.writer_sw_ver,
            sizeof(const_writer_info_default.writer_sw_ver)) != 0)
        {

            JYMC_LOG_WARN("SYS_INFO sw_ver mismatch, update to default value");
            memcpy(&p_st_sys_info->st_writer_info.writer_sw_ver, const_writer_info_default.writer_sw_ver,
                sizeof(const_writer_info_default.writer_sw_ver));
            endian_reverse_buff((uint8_t*)&p_st_sys_info->st_writer_info.writer_sw_ver,
                sizeof(p_st_sys_info->st_writer_info.writer_sw_ver));

            /* re-calc crc16 */
            p_st_sys_info->sys_info_crc16 = ftd_utils_hw_crc16((uint8_t*)p_st_sys_info,
                sizeof(SYS_INFO) - sizeof(p_st_sys_info->sys_info_crc16));
...
{
    p_fw_info->deploy_id                        = endian_reverse_u32(p_fw_info->deploy_id);
    p_fw_info->chip_id                          = endian_reverse_u32(p_fw_info->chip_id);
    p_fw_info->chip_partition_ope_bitmap        = endian_reverse_u32(p_fw_info->chip_partition_ope_bitmap);
    ...
    p_fw_info->chip_config_data_start_addr      = endian_reverse_u32(p_fw_info->chip_config_data_start_addr);
    ...
    p_fw_info->st_config_info.size              = endian_reverse_u16(p_fw_info->st_config_info.size);
    p_fw_info->st_config_info.config_crc16      = endian_reverse_u16(p_fw_info->st_config_info.config_crc16);
    ...
    //endian_reverse_buff((uint8_t*)p_fw_info->st_pid_info.easy_mesh_pid, sizeof(p_fw_info->st_pid_info.easy_mesh_pid));
    //endian_reverse_buff((uint8_t*)p_fw_info->st_pid_info.reserved, sizeof(p_fw_info->st_pid_info.reserved));
...
void ftd_mw_sys_info_manager_reverse_endian_sys_info(SYS_INFO* p_sys_info)
{
    ...
    {
        ftd_mw_sys_info_manager_reverse_endian_fw_info(&p_sys_info->st_fw_info[i]);
    }   
...
void ftd_mw_sys_info_manager_fw_info_dump(FW_INFO* p_fw_info)
{
    ...
    JYMC_LOG_RAW(" deploy_id:                          0x%08x \n chip_id:                            0x%08x \n chip_partition_ope_bitmap :         0x%08x \n chip_code_partition_start_addr :    0x%08x \n chip_code_partition_size :          0x%08x \n chip_config_data_start_addr :       0x%08x \n chip_authcode_partition_addr :      0x%08x \n",
        p_fw_info->deploy_id, p_fw_info->chip_id, p_fw_info->chip_partition_ope_bitmap, p_fw_info->chip_code_partition_start_addr, p_fw_info->chip_code_partition_size, p_fw_info->chip_config_data_start_addr, p_fw_info->chip_authcode_partition_addr);

    JYMC_LOG_RAW(" [BIN_INFO]\n bin_info.size :                     0x%08x \n bin_info.allow_write_counts :       0x%08x \n bin_info.remain_counts :            0x%08x \n",
    ...
    JYMC_LOG_RAW(" bin_info.reserved :                 0x%04x  \n bin_info.bin_crc16 :                0x%04x \n",
        p_fw_info->st_bin_info.reserved, p_fw_info->st_bin_info.bin_crc16);

    JYMC_LOG_RAW(" [CONFIG_INFO]\n");
    JYMC_LOG_RAW(" config_info.size :                  0x%04x \n config_info.crc16 :                 0x%04x \n",
        p_fw_info->st_config_info.size, p_fw_info->st_config_info.config_crc16);

    JYMC_LOG_RAW(" [TRIPLE_INFO]\n");
    JYMC_LOG_RAW(" triple_info.size :                  0x%08x \n triple_info.allow_write_counts :    0x%08x \n triple_info.remain_counts :         0x%08x \n triple_info.reserved :              0x%04x \n triple_info.triple_crc16 :          0x%04x \n",
    ...
    JYMC_LOG_RAW(" [PID_INFO]\n pid_info.easy_mesh_pid[6] :         %02x %02x %02x %02x %02x %02x \n",
        p_fw_info->st_pid_info.easy_mesh_pid[0], p_fw_info->st_pid_info.easy_mesh_pid[1], p_fw_info->st_pid_info.easy_mesh_pid[2],
        p_fw_info->st_pid_info.easy_mesh_pid[3], p_fw_info->st_pid_info.easy_mesh_pid[4], p_fw_info->st_pid_info.easy_mesh_pid[5]);

    JYMC_LOG_RAW(" pid_info.reserved[6] :              %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
        p_fw_info->st_pid_info.reserved[0],  p_fw_info->st_pid_info.reserved[1],  p_fw_info->st_pid_info.reserved[2],  p_fw_info->st_pid_info.reserved[3],
        p_fw_info->st_pid_info.reserved[4],  p_fw_info->st_pid_info.reserved[5],  p_fw_info->st_pid_info.reserved[6],  p_fw_info->st_pid_info.reserved[7],
        p_fw_info->st_pid_info.reserved[8],  p_fw_info->st_pid_info.reserved[9],  p_fw_info->st_pid_info.reserved[10], p_fw_info->st_pid_info.reserved[11],
        p_fw_info->st_pid_info.reserved[12], p_fw_info->st_pid_info.reserved[13], p_fw_info->st_pid_info.reserved[14], p_fw_info->st_pid_info.reserved[15],
        p_fw_info->st_pid_info.reserved[16], p_fw_info->st_pid_info.reserved[17], p_fw_info->st_pid_info.reserved[18], p_fw_info->st_pid_info.reserved[19],
        p_fw_info->st_pid_info.reserved[20], p_fw_info->st_pid_info.reserved[21], p_fw_info->st_pid_info.reserved[22], p_fw_info->st_pid_info.reserved[23],
        p_fw_info->st_pid_info.reserved[24], p_fw_info->st_pid_info.reserved[25] );

    JYMC_LOG_RAW(" [ROLLCODE_INFO]\n roll_code_info.start_value :        0x%08x \n roll_code_info.end_value :          0x%08x \n roll_code_info.allow_write_counts : 0x%08x \n roll_code_info.remain_counts :      0x%08x \n",
