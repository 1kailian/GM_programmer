...
void ftd_mw_display_manager_sync_channel_burn_status(CHANNEL_BURN_STATUS* p_st_channel_burn_status)
{
    ...
    for (uint8_t i = BURN_PROCESS_BAR; i < (BURN_REMAIN_CNT + 1); i++)
    {
    ...
    if (ftd_drv_panel_uart_is_rx_complete())
    {
        ...
        switch (pannel_cmd)
        {
            ...
            case PNL2WTR_OP_BURN_MODE_AUTO:
            {
                ...
                burn_type = pannel_cmd - PNL2WTR_OP_BURN_MODE_MANUAL; // 0:manual, 1:auto
        ...
        case MCU_TO_PANEL_CMD_CHANNEL_STATUS:
        {
            ...
            switch (sub_cmd)
            {
                ...
                case BURN_REMAIN_CNT:
                {
                    ...
                    pkt_len += sizeof(g_sys_info.st_fw_info[p_st_channel_burn_status->fw_num].st_bin_info.remain_counts) * 2;
                    pannel_tx_buffer[pkt_len++] = '"';
                ...
                case BURN_CHIP_TYPE:
                {
                    ...
                    sprintf((char*)&pannel_tx_buffer[pkt_len], "%1d", g_sys_info.st_fw_info[p_st_channel_burn_status->fw_num].chip_id);

                    pkt_len += sizeof(g_sys_info.st_fw_info[p_st_channel_burn_status->fw_num].chip_id);
                    pannel_tx_buffer[pkt_len++] = '"';
