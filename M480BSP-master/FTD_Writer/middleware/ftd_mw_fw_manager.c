...
#if DEBUG_USE_UART4
#define FW_MGR_ERROR  FTD_LOG_ERROR
#define FW_MGR_WARN   FTD_LOG_WARN
#define FW_MGR_INFO   FTD_LOG_INFO
#define FW_MGR_DEBUG  FTD_LOG_DEBUG
#define FW_MGR_DEBUG_BUFF  FTD_LOG_DEBUG_BUFF
#define FW_MGR_TRACE  FTD_LOG_TRACE
#else
#define FW_MGR_ERROR
#define FW_MGR_WARN
#define FW_MGR_INFO
#define FW_MGR_DEBUG
#define FW_MGR_DEBUG_BUFF
#define FW_MGR_TRACE
#endif
...
int8_t ftd_mw_fw_manager_deploy(FW_INFO *p_st_fw_info)
{
    ...
    {
        free(p_buff);
        FW_MGR_WARN("malloc p_buff failed!");
        return -1;
    }
    #endif
    ...
    ret  = ftd_drv_flash_operation(en_flash_fw_info_name, OPERATION_ERASE, 0, flash_fw_info_size, NULL);
    FW_MGR_DEBUG("erase fw info partition[%d] ret = %d", en_flash_fw_info_name, ret);
    ret |= ftd_drv_flash_operation(en_flash_fw_bin_name, OPERATION_ERASE, 0, flash_fw_bin_size, NULL);
    FW_MGR_DEBUG("erase fw bin partition[%d] ret = %d", en_flash_fw_bin_name, ret);
    if ((0x00 != p_st_fw_info->st_triple_info.size) && (0x00 != p_st_fw_info->st_triple_info.allow_write_counts))
    {
        ret |= ftd_drv_flash_operation(en_flash_fw_triple_name, OPERATION_ERASE, 0, flash_fw_triple_size, NULL);
        FW_MGR_DEBUG("erase fw triple partition[%d] ret = %d", en_flash_fw_triple_name, ret);
    }
    if(ret != 0)
    {
        #if (defined BUFF_DYNAMIC_ALLOC)
    ...
    ret = ftd_drv_flash_operation(en_flash_fw_info_name, OPERATION_WRITE, 0, sizeof(FW_INFO), (uint8_t*)p_st_fw_info);
    FW_MGR_DEBUG("write fw info partition[%d] ret = %d", en_flash_fw_info_name, ret);
    if(ret != 0)
    {
        #if (defined BUFF_DYNAMIC_ALLOC)
        free(p_buff);
    ...
    // 4.copy fw_bin and config_bin to partition
    for (cache_read_length = 0; cache_read_length < (p_st_fw_info->st_bin_info.size + p_st_fw_info->st_config_info.size); cache_read_length += buff_size)
    {
        FW_MGR_TRACE("cache_read_length = 0x%08x", cache_read_length);
        ...
        if (ftd_drv_flash_operation(PARTITION_CACHE, OPERATION_READ, sizeof(FW_INFO) + cache_read_length, buff_size, p_buff) == 0) //success
        {
            ...
            if ((cache_read_length + buff_size) > (p_st_fw_info->st_bin_info.size + p_st_fw_info->st_config_info.size))
            {
                prog_size = (p_st_fw_info->st_bin_info.size + p_st_fw_info->st_config_info.size) % buff_size;
                FW_MGR_TRACE("prog_size = 0x%08x", prog_size);
            }
            else
            {
                prog_size = buff_size;
            ...
            // for debug only
            #if 0
            FW_MGR_RAW("READ CACHE BIN[read_len:0x%08x bin_size:0x%08x config_size:0x%08x]:\n", 
                cache_read_length, p_st_fw_info->st_bin_info.size, p_st_fw_info->st_config_info.size);
            FW_MGR_DEBUG_BUFF(p_buff, buff_size, prog_size);
            #endif
            ...
            if (ftd_drv_flash_operation(en_flash_fw_bin_name, OPERATION_WRITE, cache_read_length, prog_size, p_buff) == 0) //success
            {
                // for debug only
                #if 0
                FW_MGR_DEBUG("CHK READ %d %d offset:0x%08x size:0x%08x buff_addr:0x%08x",
        ...
        {
            FW_MGR_WARN("CACHE READ FAIL length 0x%08x fw_size 0x%08x config_size 0x%08x", 
                cache_read_length, p_st_fw_info->st_bin_info.size, p_st_fw_info->st_config_info.size);
            #if (defined BUFF_DYNAMIC_ALLOC)
            free(p_buff);
            #endif
    ...
    if ((0x00 != p_st_fw_info->st_triple_info.size) && (0x00 != p_st_fw_info->st_triple_info.allow_write_counts))
    {
        ...
        {
            if (ftd_drv_flash_operation(PARTITION_CACHE, OPERATION_READ, sizeof(FW_INFO) + p_st_fw_info->st_bin_info.size + p_st_fw_info->st_config_info.size + cache_read_length, buff_size, p_buff) == 0) //success
            {
                FW_MGR_TRACE("triple read addr:0x%08x cache_read_length = 0x%08x", (sizeof(FW_INFO) + p_st_fw_info->st_bin_info.size + p_st_fw_info->st_config_info.size + cache_read_length), cache_read_length);
                ...
                // for debug only
                #if 0
                FW_MGR_RAW("READ CACHE TRIPLE[read_len:0x%08x triple_size:0x%08x]:\n", cache_read_length, p_st_fw_info->st_triple_info.size);
                FW_MGR_DEBUG_BUFF(p_buff, buff_size, prog_size);
                #endif
    ...
    {
        FW_MGR_TRACE("CHK FW READ %d %d offset:0x%08x size:0x%08x buff_addr:0x%08x",
    ...
    if(crc16_result != p_st_fw_info->st_bin_info.bin_crc16) //fwx_bin crc16 error
    {
        ...
        free(p_buff);
        #endif
        FW_MGR_ERROR(" fw%02d_bin crc16 check FAIL [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_bin_info.bin_crc16);
        return -61;
    }
    FW_MGR_INFO(" fw%02d_bin crc16 check PASS [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_bin_info.bin_crc16);

    // 7.check config_bin crc16
    if (0x00 != p_st_fw_info->st_config_info.size)
    {
        cache_read_length = p_st_fw_info->st_bin_info.size; // offset = fw_bin_size

        FW_MGR_TRACE("CHK CONFIG READ %d %d offset:0x%08x size:0x%08x buff_addr:0x%08x",
            en_flash_fw_bin_name, OPERATION_READ, cache_read_length, p_st_fw_info->st_config_info.size, p_buff);

        if (ftd_drv_flash_operation(en_flash_fw_bin_name, OPERATION_READ, cache_read_length, p_st_fw_info->st_config_info.size, p_buff) == 0) //success
        {
            //ftd_utils_hw_crc16_update(p_buff, buff_size);

            crc16_result = crc16_ccitt_update(0xFFFF, (uint8_t*)p_buff, p_st_fw_info->st_config_info.size);

            if (crc16_result != p_st_fw_info->st_config_info.config_crc16) //fwx_triple crc16 error
            {
                #if (defined BUFF_DYNAMIC_ALLOC)
                free(p_buff);
                #endif
                FW_MGR_ERROR(" fw%02d_config crc16 check FAIL [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_config_info.config_crc16);
                return -70;
            }
            FW_MGR_INFO(" fw%02d_config crc16 check PASS [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_config_info.config_crc16);
        }
        else
        {
            #if (defined BUFF_DYNAMIC_ALLOC)
    ...
    if ((0x00 != p_st_fw_info->st_triple_info.size) && (0x00 != p_st_fw_info->st_triple_info.allow_write_counts))
    {
        ...
        {
            FW_MGR_ERROR(" fw%02d_triple_info size error[%d] ttl_write_cnt[%d] triple_len[%d]", 
        ...
        if(crc16_result != p_st_fw_info->st_triple_info.triple_crc16) //fwx_triple crc16 error
        {
            ...
            free(p_buff);
            #endif
            FW_MGR_ERROR(" fw%02d_triple crc16 check FAIL [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_triple_info.triple_crc16);
            return -82;
        }
        FW_MGR_INFO(" fw%02d_triple crc16 check PASS [0x%x][0x%x]", p_st_fw_info->fw_region, crc16_result, p_st_fw_info->st_triple_info.triple_crc16);
    }

    // 9.write fw_info to partition
    ...
    st_sys_info.st_fw_info[p_st_fw_info->fw_region - 1].deploy_flag = 1; // set deloy flag
    ...
    if (ftd_mw_sys_info_manager_save_nv())
        return -100; // [NOTE] int8 min value -127
