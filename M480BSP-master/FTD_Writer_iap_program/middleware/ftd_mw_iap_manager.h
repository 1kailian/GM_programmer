...
#define APP_2_END_BASE   0x7F000 //app2 size : 0x30000 (192kb)
...
typedef struct
{
    ...
    uint8_t  page_dirty;          // Whether there is unwritten data
    uint16_t crc16;               // CRC16 value
    uint8_t upgrade_flag;          // Upgrade flag
} IAP_FMC_T;


#define IAP_MAGIC  0x4A594D43  // 'JYMC'
#define CHECK_LEN_BYTES   500U
#define IAP_MODE_TIME_OUT  60000 //60s

#ifdef __ICCARM__
#pragma data_alignment=4
typedef struct
{
    uint32_t start_magic;  // Fixed magic number, determine if it is valid.
...
void ftd_mw_iap_manager_erase_app_2(void); // Erase APP2 area
