...
static void ftd_drv_burn_channel1_rx_start(uint8_t *buf, uint16_t len)
{
    ...
    while ((!(UART1->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk)) && (timeout > 0)) {
        volatile uint32_t dummy = UART1->DAT;
        (void)dummy;
        timeout--;
    }
    ...
    /* Open PDMA Channel */
    PDMA_Open(PDMA,1 << UART1_RX_PDMA_CH); // Channel 1 for UART1 RX
    // Select basic mode
...
static void ftd_drv_burn_channel1_tx_start(uint8_t *buf, uint16_t len)
{
    ...
    UART_Open(UART1, 115200);
    /* Open PDMA Channel */
    PDMA_Open(PDMA, 1 << UART1_TX_PDMA_CH); // Channel 1 for UART1 RX
    // Select basic mode
...
static void ftd_drv_burn_channel2_rx_start(uint8_t *buf, uint16_t len)
{
    ...
    uint32_t timeout = 10000;
    while ((!(UART2->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk)) && (timeout > 0))
    {
        volatile uint32_t dummy = UART2->DAT;
        (void)dummy;
        timeout--;
    }
    ...
    /* Open PDMA Channel */
    PDMA_Open(PDMA, 1 << UART2_RX_PDMA_CH); // Channel 1 for UART RX
    // Select basic mode
...
static void ftd_drv_burn_channel2_tx_start(uint8_t *buf, uint16_t len)
{
    ...
    /* Open PDMA Channel */
    PDMA_Open(PDMA,1 << UART2_TX_PDMA_CH); // Channel 1 for UART1 RX
    // Select basic mode
...
static void ftd_drv_burn_channel4_rx_start(uint8_t *buf, uint16_t len)
{
    ...
    uint32_t timeout = 10000;
    while ((!(UART4->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk)) && (timeout > 0))
    {
        volatile uint32_t dummy = UART4->DAT;
        (void)dummy;
        timeout--;
    }
    ...
    /* Open PDMA Channel */
    PDMA_Open(PDMA, 1 << UART4_RX_PDMA_CH); // Channel 1 for UART1 RX
    // Select basic mode
...
static void ftd_drv_burn_channel4_tx_start(uint8_t *buf, uint16_t len)
{
    ...
    /* Open PDMA Channel */
    PDMA_Open(PDMA,1 << UART4_TX_PDMA_CH); // Channel 1 for UART1 RX
    // Select basic mode
...
void ftd_drv_burn_channel_start(uint8_t *tx_buf, uint16_t tx_len, uint8_t *rx_buf, uint16_t rx_len)
{
    ...
    if (timeout == 0) {
        FTD_LOG_DEBUG("ftd_drv_burn_channel_start clear fifo timeout");
    }
