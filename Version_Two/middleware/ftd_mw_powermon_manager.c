 ...
 *       - LSB = 2.5 ?V/bit = 0.0025 mV/bit
 *       - Formula: Voltage(mV) = Raw_Value �� LSB �� 1000
 *
 *       Two's Complement Examples:
 *       - Positive: +80mV �� +32000 (0x7D00)
 *       - Negative: -80mV �� -32000 (0x82FF in two's complement)
 *       - Zero: 0mV �� 0 (0x0000)
 */
float ftd_mw_powermon_manager_get_shunt_voltage(uint8_t u8SlaveAddr)
{
    ...
    // Calculation: shunt_voltage(mV) = raw_value �� 0.0025
...
int8_t ftd_mw_powermon_manager_get_burn_current_status(uint8_t burn_ch, float current_min, float current_max)
{
    ...
    switch (burn_ch)
    {
        case 0:
        {
            ...
            FTD_LOG_DEBUG("INA236 current: %.2f, range: %.2f-%.2f\n", current, current_min, current_max);
...
int8_t ftd_mw_powermon_manager_get_burn_bus_voltage_status(uint8_t burn_ch, float voltage_target, float voltage_offset)
{
    ...
    switch (burn_ch)
    {
        case 0:
        {
            ...
            FTD_LOG_DEBUG("INA236 bus voltage: %.2f, target: %.2f, offset: %.2f\n", bus_voltage, voltage_target, voltage_offset);
            if (bus_voltage < (voltage_target + voltage_offset) && bus_voltage >(voltage_target - voltage_offset))
            {
                ret = 0;
            }
            else
...
{
    int ret = 0;
    FTD_LOG_DEBUG("burn ch %d\n", burn_ch);
    switch (burn_ch)
    {
        case 0:
        {
            int current_status = ftd_mw_powermon_manager_get_burn_current_status(burn_ch, POWERMON_CURRENT_MIN, POWERMON_CURRENT_MAX);
            int bus_voltage_status = ftd_mw_powermon_manager_get_burn_bus_voltage_status(burn_ch, POWERMON_BUS_VOLTAGE, POWERMON_BUS_VOLTAGE_OFFSET);
...
/**
 * @brief  Read current value for specific channel
 * @param burn_ch  Burn channel number (0-3)
...
float ftd_mw_powermon_manager_get_channel_current(uint8_t burn_ch)
{
    ...
    {
        case 0:
            current_value = ftd_mw_powermon_manager_get_current(POWERMON_SAVE_ADDR_1);
            break;
        case 1:
            current_value = ftd_mw_powermon_manager_get_current(POWERMON_SAVE_ADDR_2);
            break;
        case 2:
            current_value = ftd_mw_powermon_manager_get_current(POWERMON_SAVE_ADDR_3);
            break;
        case 3:
            current_value = ftd_mw_powermon_manager_get_current(POWERMON_SAVE_ADDR_4);
            break;
        default:
            current_value = 0.0f;
            FTD_LOG_WARN("Invalid burn channel: %d", burn_ch);
            break;
    }

    FTD_LOG_INFO("Channel %d current: %.2f mA", burn_ch, current_value);
