 ...
 *
 */
static void ftd_mw_hc_manager_checksum(uint8_t* p_buf)
{
    uint32_t i;
    uint8_t checksum = 0;
    uint32_t len = 0;
    ...
    // if(ftd_mw_hc_manager_check_data(p_buffer) >= 0)
    {
        ...
        switch (p_buffer[FRAME_HEADER_SIZE])
        {
            ...
            case 0x54:
            {
                ...
                if ((extend_cmd == 0x000D) && (p_buffer[ext_data_type_offset] == 0x09))
                    en_event_type = FTD_PROG_EVENT_DEPLOY_SEG_DATA_START;
                else if ((extend_cmd == 0x000D) && (p_buffer[ext_data_type_offset] == 0x0A))
                    en_event_type = FTD_PROG_EVENT_FW_UPDATE_SEG_DATA_START;
                else if ((extend_cmd == 0x000E) && (p_buffer[ext_data_type_offset] == 0x09))
                    en_event_type = FTD_PROG_EVENT_DEPLOY_SEG_DATA;
                else if ((extend_cmd == 0x000E) && (p_buffer[ext_data_type_offset] == 0x0A))
                    en_event_type = FTD_PROG_EVENT_FW_UPDATE_SEG_DATA;
                else if (extend_cmd == 0x0045)
                {
                    extend_data_type = (p_buffer[ext_data_type_offset] << 8) | p_buffer[ext_data_type_offset + 1];
                    switch (extend_data_type)
                    {
                        case 0x0002:
                            en_event_type = FTD_PROG_EVENT_GET_SYSINFO;
                            break;
                        case 0x0003:
                            en_event_type = FTD_PROG_EVENT_GET_CRC_RESAULT; //0x0003 is crc report cmd
                            break;
                        case 0x0007:
                            en_event_type = FTD_PROG_EVENT_SET_WRITER_INFO; //0x0007 is writer info set cmd
                            break;
                        case 0x0008:
                            en_event_type = FTD_PROG_EVENT_BURN_START_TEST_TMP; //0x0008 temp debug use
                            break;
                        case 0x0009:
                            en_event_type = FTD_PROG_EVENT_BURN_START_TMP; //0x0008 temp debug use
                        default:
                            break;
                    }

                }
                else if (extend_cmd == 0x0031)
                {
                    en_event_type = FTD_PROG_EVENT_ENTER_IAP_MODE;
                }
                else
                {
                }
...
int8_t ftd_mw_hc_manager_get_seg_pkt_data(uint8_t* p_buf, uint16_t* p_cur_param_len, uint16_t* p_cur_seg_num)
{
    ...
    FTD_LOG_TRACE("ext_cmd %04x data_type %02x", extend_cmd, p_buf[data_type_offset]);
    ...
    if ((extend_cmd == 0x000E) && (p_buf[data_type_offset] == 0x09)) //seg packet data/writer deploy file
    {
        ...
        *p_cur_seg_num = (p_buf[data_type_offset + 1] << 8) | p_buf[data_type_offset + 2];
        FTD_LOG_TRACE("seg_num %04x", *p_cur_seg_num);
        return 0;
    }
    return -1;
}
